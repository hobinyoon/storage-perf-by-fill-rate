...
def Setup():
	with Cons.MeasureTime("Setting up local SSD ..."):
		...
		# -o discard for TRIM
		Util.RunSubp("sudo mount -t ext4 -o discard /dev/xvdb /mnt/local-ssd1")
		Util.RunSubp("sudo chown -R ubuntu /mnt/local-ssd1")
...
def Run():
	...
	with Cons.MeasureTime("Setting up ..."):
		...
		Util.RunSubp("dd if=/dev/zero of=%s/zeros bs=1M count=100 > /dev/null 2>&1" % dn_rand_data)
...
def _MeasurePerf():
	...
	cmd = "(date +\"%%y%%m%%d-%%H%%M%%S\" >> %s)" \
			...
			" && (ioping -D -p 100 -c 100 -i 0 -WWW         -q /mnt/local-ssd1/ioping-tmp >> %s)" \
			" && (ioping -D -p   1 -c   1 -i 0 -WWW -s 100m -q /mnt/local-ssd1/ioping-tmp >> %s)" \
			" && (df /dev/xvda1 /dev/xvdb >> %s)" \
